	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			...
			if tc.expErr {
				...
				s.Require().Equal(len(res.GetResult().GetEvents()), 6)
				s.Require().True(res.GetGasInfo().GetGasUsed() > 0)
			}
		})
	}
}
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.expErr {
				...
				s.Require().Equal(len(result.GetResult().GetEvents()), 6)
				s.Require().True(result.GetGasInfo().GetGasUsed() > 0)
			}
		})
	}
}
